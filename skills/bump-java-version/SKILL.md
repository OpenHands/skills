...
description: Migrate a Maven project from one Java LTS to the next (8->11, 11->17, 17->21, 21->25) so it still compiles under the new JDK and previously-passing tests still pass, using only standard tools (JDKs, Maven, and OpenRewrite recipes from Maven Central; no project-specific scripts). Use when upgrading or bumping the Java version of a Maven project, modernizing to a newer JDK or LTS, or doing the Spring Boot 2->3 / javax->jakarta migration a Java upgrade requires.
license: MIT
compatibility: Requires Maven (or the project's mvnw), the JDKs for the hop, and network access to Maven Central for the OpenRewrite recipes.
triggers:
  - bump java version
  - upgrade java
  - java lts migration
